...
export class GatewayBasicConfigurationComponent implements OnDestroy, ControlValueAccessor, Validators {
  ...
  writeValue(basicConfig: GatewayConfigValue): void {
    ...
    this.checkAndFetchCredentials(basicConfig?.sobeam?.security ?? {} as GatewayConfigSecurity);
    if (basicConfig?.grpc) {
      this.toggleRpcFields(basicConfig.grpc.enabled);
    }
    const commands = basicConfig?.sobeam?.statistics?.commands ?? [];
  ...
  commandFormArray(): FormArray {
    return this.basicFormGroup.get('sobeam.statistics.commands') as FormArray;
  }
  ...
  private removeAllSecurityValidators(): void {
    const securityGroup = this.basicFormGroup.get('sobeam.security') as FormGroup;
  ...
  private openConfigurationConfirmDialog(): void {
    this.deviceService.getDevice(this.device.id).pipe(takeUntil(this.destroy$)).subscribe(gateway => {
      ...
      (GatewayRemoteConfigurationDialogComponent, {
        ...
        (res) => {
          if (!res) {
            this.basicFormGroup.get('sobeam.remoteConfiguration').setValue(true, {emitEvent: false});
          }
        }
      );
    });
  ...
  private initBasicFormGroup(): void {
    this.basicFormGroup = this.fb.group({
      sobeam: this.initSobeamFormGroup(),
  ...
  private initSobeamFormGroup(): FormGroup {
  ...
  private observeSecurityPasswordChanges(): void {
    const securityUsername = this.basicFormGroup.get('sobeam.security.username');
    this.basicFormGroup.get('sobeam.security.password').valueChanges.pipe(takeUntil(this.destroy$)).subscribe(password => {
  ...
  private observeRemoteConfigurationChanges(): void {
    this.basicFormGroup.get('sobeam.remoteConfiguration').valueChanges.pipe(takeUntil(this.destroy$)).subscribe(enabled => {
      if (!enabled) {
  ...
  private observeDeviceActivityChanges(): void {
    const checkingDeviceActivityGroup = this.basicFormGroup.get('sobeam.checkingDeviceActivity') as FormGroup;
  ...
  private updateSecurityType(security, credentials: DeviceCredentials): void {
    ...
    if (securityType) {
      this.basicFormGroup.get('sobeam.security.type').setValue(securityType, { emitEvent: false });
    }
  }
  ...
  private updateAccessTokenCredentials(credentials: DeviceCredentials, security: GatewayConfigSecurity): void {
    this.basicFormGroup.get('sobeam.security.accessToken').setValue(credentials.credentialsId, { emitEvent: false });
    if (security.type === SecurityTypes.TLS_ACCESS_TOKEN) {
      this.basicFormGroup.get('sobeam.security.caCert').setValue(security.caCert, { emitEvent: false });
    }
  }
  ...
  private updateMqttBasicCredentials(credentials: DeviceCredentials): void {
    ...
    this.basicFormGroup.get('sobeam.security.clientId').setValue(parsedValue.clientId, { emitEvent: false });
    this.basicFormGroup.get('sobeam.security.username').setValue(parsedValue.userName, { emitEvent: false });
    this.basicFormGroup.get('sobeam.security.password').setValue(parsedValue.password, { emitEvent: false });
  }
}
